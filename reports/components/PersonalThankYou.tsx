...
export function PersonalThankYou({ thankYou, assets }: PersonalThankYouProps) {
  ...
  return (
    <section className="flex items-start gap-7">
      <div className="relative aspect-[4/5] w-[2.55in] shrink-0 overflow-hidden rounded-[4px] bg-subtle">
        <img
          src={assets.founder}
      ...
      <div className="pt-2">
        ...
        <h2 className="max-w-sm font-display text-[2.15rem] font-bold leading-[0.95] tracking-[-0.02em] text-dark">
          {thankYou.headline ?? "Thanks for testing this out."}
        </h2>
        {thankYou.body ? (
          <p className="mt-4 max-w-sm text-[16px] leading-relaxed text-muted">{thankYou.body}</p>
        ) : null}
        <p className="mt-5 font-display text-sm font-bold tracking-wide text-dark">Ben</p>
